...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            image_index = (self.age - 1) % self.lifespan
            canvas.draw_image(self.image, [self.image_center[0] + image_index * self.image_size[0], 
...
def rock_spawner():
    global rock_group
    if started and len(rock_group) < MAX_NUM_ROCKS:
        while True:
            rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
            rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
            rock_vel[0] *= (1 + score / 10.0)
            rock_vel[1] *= (1 + score / 10.0)
            rock_avel = random.random() * .2 - .1
            new_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
            if dist(new_rock.get_position(), my_ship.get_position()) <= new_rock.get_radius() + my_ship.get_radius():
                continue
            else:
                rock_group.add(new_rock)
                break

# helper function that updates and draws a group of sprites
...
# helper function that checks for collisions between a group and another object
def group_collide(group, other_object):
    global explosion_group
    is_collide = False
    for group_member in set(group):
        if group_member.collide(other_object):
            is_collide = True
            new_explosion = Sprite(group_member.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(new_explosion)
            group.remove(group_member)
    return is_collide

# help function that checks for collisions between two groups
def group_group_collide(group1, group2):
    num_collisions = 0
    for group1_member in set(group1):
        if group_collide(group2, group1_member):
